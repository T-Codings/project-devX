...
export default function ProvenRecordsSection() {
  ...
  return (
    <section
      ref={ref}
      className="flex flex-col items-center text-center mt-[120px] bg-purple-50 h-[350px]"
    >
      <div>
        <h3 className="text-[36px] font-bold font-sans pt-[80px]">
          Here are a few numbers that we are proud of
        </h3>
      </div>

      <div className="flex flex-row gap-[80px] ml-1">
        {stats.map((s) => (
          <div key={s.label} className="flex flex-row items-center text-center">
            <img className={s.iconClass} src={s.icon} alt={s.alt} />
            <div className="flex flex-col justify-center items-start text-center">
              <h4 className="mt-4 ml-[5px] font-bold text-[20px] font-sans">
                {formatCount(s.value)}+
              </h4>
              <p className="mt-[-15px] ml-[8px] text-purple-600 text-[18.5px] font-bold font-sans">
                {s.label}
              </p>
            </div>
          </div>
